...
void setMotorsRaw(int pwmL, int pwmR){
  pwmL = applyDeadzone(pwmL);
  pwmR = applyDeadzone(pwmR);
  digitalWrite(PIN_Motor_AIN_1, pwmL >= 0);
  digitalWrite(PIN_Motor_BIN_1, pwmR >= 0);
  analogWrite(PIN_Motor_PWMA, clampi(abs(pwmL), 0, 255));
  analogWrite(PIN_Motor_PWMB, clampi(abs(pwmR), 0, 255));
  last_pwmL = pwmL;
  last_pwmR = pwmR;
}
